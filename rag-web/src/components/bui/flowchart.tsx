 ...
 *
 * 10.7 适配：PURPLE/AMBER hex → --purple/--orange 令牌；
 * 阴影 rgba → color-mix（R4 合规）。
 * ───────────────────────────────────────────────────────── */
...
export default function Flowchart() {
  ...
  /* ── dragging ── */
  const onPointerDown = (node: StepNode) => (event: ReactPointerEvent<HTMLDivElement>) => {
  ...
  const onPointerMove = (node: StepNode) => (event: ReactPointerEvent<HTMLDivElement>) => {
            ...
            {node.condition ? (
              ...
              <button
                ...
                className={`w-full cursor-pointer rounded-card bg-surface text-left outline-none
                  ...
                  ${
                    active
                      ? "shadow-[0_0_0_1.5px_var(--accent),0_2px_10px_color-mix(in_srgb,var(--ink)_5%,transparent)]"
