 ...
 *
 * 10.7 适配：posthog 埋点剥离（J24，移除 capture 调用）；
 * rgba 阴影 → shadow-btn 令牌（R4 合规）。
 * ───────────────────────────────────────────────────────── */
    ...
    <div className="flex h-[288px] w-full max-w-95 flex-col self-start overflow-hidden rounded-[14px] bg-surface shadow-card">
      ...
      <div className="mt-auto shrink-0 p-1.5">
        <div
          ...
          className="flex cursor-text flex-col gap-2 rounded-control border border-line bg-field p-2.5 shadow-btn transition-[border-color,box-shadow] duration-150 focus-within:border-line-strong"
        >
          <input
            ref={inputRef}
            value={draft}
