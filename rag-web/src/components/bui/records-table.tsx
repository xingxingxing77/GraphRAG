 ...
 *
 * 10.7 适配：GlideMenu → 本地基元；React.* 类型显式导入。
 * ───────────────────────────────────────────────────────── */
...
function Tag({ name }: { name: string }) {
  ...
  return (
    <span
      ...
      style={{ "--tag-base": color.base } as CSSProperties}
    >
      {name}
    </span>
  );
...
function HeaderCell({ label, icon, sortKey, sort, onSort, onResizeStart, resizing = false, selected = false, onPick }: { label: string; icon: ReactNode; sortKey?: SortKey; sort: { key: SortKey; dir: 1 | -1 }; onSort: (key: SortKey) => void; onResizeStart: (event: ReactPointerEvent<HTMLSpanElement>) => void; resizing?: boolean; className?: string; selected?: boolean; onPick?: (event: ReactMouseEvent) => void }) {
  return (
    <th className={`records-header-cell ${selected ? "is-colsel" : ""}`}>
...
export default function RecordsTable({ fill = false }: { fill?: boolean; variant?: string }) {
  ...
  const openProp = (col: string) => (event: ReactMouseEvent) => {
  ...
  const startColumnResize = (key: ColumnKey, minWidth = 120) => (event: ReactPointerEvent<HTMLSpanElement>) => {
    event.preventDefault();
    event.stopPropagation();
    setProp(null);
    setConfigMenu(null);
