 ...
 *
 * 10.7 适配：官方 data-URI 演示头像含硬编码 hex（R4 禁），
 * 换为令牌底色字母头像；图标数组 ReactNode 化。
 * ───────────────────────────────────────────────────────── */
...
/* 来源演示数据：头像以令牌底色 + 首字母呈现（R4 合规替代 data-URI） */
const SOURCES = [
  { name: "Scoop Data", domain: "scoopdata.io", href: "https://scoopdata.io/", initial: "S", tone: "bg-green" },
  { name: "Trends Index", domain: "trends.google.com", href: "https://trends.google.com/trends/", initial: "T", tone: "bg-accent" },
  { name: "Market Basket", domain: "marketbasket.io", href: "https://marketbasket.io/", initial: "M", tone: "bg-orange" },
];

function SourceAvatar({ source, className = "" }: { source: (typeof SOURCES)[number]; className?: string }) {
  return (
    <span className={`source-avatar ${source.tone} text-white ${className}`}>{source.initial}</span>
  );
}
          ...
          <span className="flex -space-x-1">
            {SOURCES.map((source) => (
              <SourceAvatar
                key={source.domain}
                source={source}
                className="size-3.5 rounded-full shadow-[0_0_0_1.5px_var(--canvas)]"
              />
            ))}
          </span>
