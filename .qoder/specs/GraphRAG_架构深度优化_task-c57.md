...
### 运行时时序图 -- v2 新增 (D2)，v2.2 标注双服务边界

上图为静态分层视图，实际运行时控制流如下。**注意三处与直觉不同的地方：记忆注入在查询理解之前；Reranker 在 Agent 循环之外一次性执行；主链路整体运行在 langgraph-server 内，FastAPI 只承担链路外围的业务面职责（J19）**。
...
- v2.x 曾设计的"后端 SSE 回放器"已随 J20 取消——server 流出的是节点事件与终态，模拟流式的打字机渲染由前端实现
- chitchat「直答」步在图内走 ToolRouter → Generator 直连边，不进入反思循环（J9 的边路由消解）；deep 档无依赖子步骤的并行扇出在 ToolRouter 内部完成（A1），对上游时序透明
- 反思短路（A2）是代码级判定而非 LLM 调用——Top-K 平均 Rerank 分超阈值或证据达标时直接路由 Generator，Reflector 节点被跳过

---

## 三、核心数据契约 -- **v2 新增 (D1/D2)**

> 数据契约是各层之间唯一的交接物定义，所有层的输入输出必须符合本章模型，禁止私自扩展字段。代码落地位置：`app/core/models.py`。
>
> **v1 缺口警示**：v1 仅提及模型名称而未定义字段，已导致代码骨架出现类型漂移（清洗规则接收 `ParsedDocument` 而非声明的 `CleanedDocument`）。v2 起以本章为准。
...
class CleanedDocument(BaseModel):
    """P3 清洗层输出。注意: 清洗规则的输入输出均为本类型(v2 修正类型漂移)"""
    doc_id: str; schema_version: Literal["1"] = "1"
    text: str
...
| token_budget_exhausted | bool ★ | v2.2 新增：B4 预算感知调度开关——wall-clock/token 任一预算耗尽即置位，路由直入 Generator 降级作答 |
| tool_call_cache | dict[str, RetrievalResult] | v2.2 新增：D11 run 内工具调用记忆化缓存，key 为 (tool, query) 规范 hash，防止重复检索 |
| reflect_feedback | ReflectFeedback \| None | v2.2 新增：Reflector 结构化输出 {sufficient, missing_aspects[], followup_queries[]}，回环时 Planner 增量补计划的依据 |
...
### 3.6 业务面 REST API v1 规范 -- **v2.2 新增 (J19)**

FastAPI :8000 承载的非流式端点全集（聊天主链路在 Agent 面，见第二章运行时时序图）：
...
| POST | /feedback | 点赞/点踩上报（第六章在线评估闭环的数据源） | JWT |
...
- ChatRequest 补齐字段（v2.2）：`latency_tier: fast|standard|deep`（缺省 auto 由意图路由定档）、`model: str | None`（J2 覆盖 generator 角色默认条目）；ChatResponse 补 `degraded: bool`、`latency_tier`

---

## 四、各层详细设计

### 第 1 层：网关层（Gateway Layer） -- **v2.2 重构为业务面（J19）**

**职责**: 业务面 API 入口、安全、非流式数据服务。聊天主链路已移交 langgraph-server（SDK 直连），本层不再承载流式响应。
...
> CORS 注意：`allow_origins=["*"]` 与 `allow_credentials=True` 组合被浏览器规范禁止——必须配置显式 origin 白名单（开发期含 Vite dev 源 `http://localhost:5173`），生产收紧。

**项目结构**:
```
app/api/
  endpoints/
    chat.py          # POST /chat/precheck - 语义缓存短路 (v2.2 收缩)
    auth.py          # POST /auth/token - JWT 签发 (v2.2 新增)
    sessions.py      # 会话列表与历史消息 (v2.2 新增)
    feedback.py      # 反馈上报 (v2.2 新增)
    graph.py         # 图谱子图代理 (v2.2 新增)
    config_public.py # 公共配置下发 (v2.2 新增)
...
### 第 2 层：查询理解层（Query Understanding Layer） -- **新增**

**职责**: 在检索前对查询进行预处理，显著提升召回质量

| 组件 | 说明 |
|------|------|
| 意图路由器 (Query Router) | 分类查询意图：简单事实型 / 多跳推理型 / 对比型 / 总结全局型 / 闲聊型，决定检索策略组合与延迟档位（见下文「延迟预算与分级策略」） |
...
**关键决策（v2 · M2 裁决）-- 四组件合并为单次结构化调用**:

意图分类、改写、分解、实体抽取若各自串行调用 LLM，本地 7B 需 3-6s 纯开销。v2 定案：**一次 LLM 调用返回全部结果**，延迟降至 1-2s。
...
- HyDE 从默认链路中移除，降级为 `deep` 档位的可选增强项
- `intent=chitchat` 走快速路径：跳过整个检索链路直接生成
...
**性能要求**: 所有检索器并行执行（asyncio.gather），总延迟取最慢一路；每个外部调用设独立超时（见第九章降级矩阵），单路失败不阻塞其他路。

**接口契约**: 所有检索器实现第三章 `BaseRetriever` Protocol，输入输出统一为 `RetrievalResult`。

**ES ↔ Neo4j 协同检索流程（v2.1 · J6 定案）**:
...
app/retrieval/
  ...
  es_retriever.py       # Elasticsearch 全文检索 + 图谱协同 (v2.1 更名自 fulltext_retriever)
...
### 第 5 层：Reranker 精排层（Reranking Layer） -- **新增（核心优化）**

**职责**: 对粗排结果进行精细重排序，剔除噪音，压缩上下文

| 组件 | 技术 | 说明 |
|------|------|------|
| Cross-Encoder 重排序 | BGE-Reranker-v2-m3（进程内 FlagEmbedding） | 对 (query, passage) 对进行相关性打分，比 Bi-Encoder 精度高 |
| 分数阈值过滤 | 配置化阈值（如 score > 0.3） | 过滤掉低相关性结果 |
| 上下文压缩器（J11 可插拔） | 策略接口: llm_extract / extractive / none | 默认 deep 档 llm_extract（LLM 提取与查询相关的核心句），其余档位 none；策略经 `retrieval.compression_strategy` 配置切换，**不写死**，后续可整体替换实现 |
| Top-K 截断 | K 可配置（如 K=5） | 最终送入 Agent 的证据数量 |

**BGE-Reranker 部署要点（v2 · H1 修正）**:

> ⚠️ **Ollama 不支持 Cross-Encoder Reranker**——Ollama 仅有 generate / embedding 两类端点，无 rerank API。v1 中"通过 Ollama 部署"的表述是技术选型错误，v2 定案如下：
...
- 性能: 计算密集型操作；与主 LLM 共享显存时 int8 量化后预留 ≥2GB；并发请求经全局 semaphore 串行化（见第九章）
...
**LangGraph 状态图（v2 修正：消除 v1 图中 Self-Correction 位置矛盾；v2.2 增强：反思短路 A2 / 直答直连边 / 并行扇出 A1）**:
```
START
  |
  v
...
| 节点 | 模型选择 | 职责 |
|------|----------|------|
| Planner | 角色化模型条目（J2，默认可配本地轻量模型） | 分析问题，制定检索和推理计划，输出 `list[PlanStep]`（契约见下文）。**v2.1 · J9：所有查询均经过 Planner**；chitchat 类的计划退化为「直答」单步（`tool="direct_answer"`，代码分支零 LLM 调用），以此与第 2 层快速路径消解一致。**v2.2：回环时基于 Reflector 的 `followup_queries[]` 增量补计划，而非全量重规划** |
| Tool Router（兼执行） | -- (代码逻辑) | **v2 定案：路由与执行合一，不设独立 Executor 节点**（与代码骨架 `app/agent/nodes/` 目录一致），结果写入 retrieved_evidence。**v2.2 增强：① deep 档对 `depends_on=[]` 的步骤经 LangGraph Send API 并行扇出，fan-in 合并去重（A1）；② run 内 `(tool, query)` hash 记忆化，重复调用直接复用缓存（D11）；③ 「直答」步不执行任何工具并走 Generator 直连边** |
| Reflector | Qwen2.5-7B (快) | 评估已有信息是否充分；受轮次预算约束。**v2.2 结构化输出契约：`{sufficient: bool, missing_aspects: string[], followup_queries: string[]}`**——sufficient 驱动回环路由，followup_queries 作为下一轮补检查询。入口前置代码级短路判定（A2），高置信证据时本节点整体跳过 |
| Generator | Qwen2.5-32B/72B | 基于检索证据生成最终回答（缓冲式完整生成，见第 7 层）。**v2.2 · D9'：prompt 组装按 Rerank 分将高置信证据置于首尾（抗 lost-in-the-middle），引用编号随排序同步重编** |
| Self-Correction | Qwen2.5-7B | 忠实度校验：检查生成内容与检索证据的一致性；**失败原因（无证据支撑的句子清单）作为约束注入重生成 Prompt，提升二次通过率** |

**PlanStep 契约 -- v2.2 补齐（3.4 此前引用但未定义；实现落地于 `app/core/models.py`）**:
...
| wall_clock_budget -- v2.2 新增 | fast 8s / standard 22s / deep 45s（`reliability.yaml`） | 单请求挂钟时间上限（略高于 D4 各档 P95 目标，留降级余量）；配合 B4 入口预检执行，超限时跳过剩余反思/校验环节强制生成 |
| LangGraph recursion_limit | 15 | 框架级硬保护，编译时配置，超限抛 GraphRecursionError |

> 原则：超限行为是"降级回答"而非抛错——始终给用户一个带置信度标注的答案。
> **节点入口预算预检（v2.2 · B4）**：每个 LLM/工具节点执行前检查累计 token_usage 与剩余 wall-clock；任一预算不足以支撑完整反思/重试时置 `token_budget_exhausted=true` 并路由直入 Generator 降级作答。`recursion_limit` 仅作框架级最后防线，正常路径不应触达。
> AgentState 完整字段定义见第三章 3.4；条件边路由函数依赖其中带 ★ 的字段。

**Agent 层运行效率优化 -- v2.2 新增**:
...
| D9' | 上下文抗失序排序 | Generator 组装证据时按 Rerank 分将高置信证据置于 prompt 首尾（lost-in-the-middle 对策）；引用编号随排序重编并同步 Citation 列表 | --（默认启用） |
| D10' | HITL 中断预留 | 图内预置 `interrupt()` 挂点（默认关闭）：deep 档可在高成本操作（Web 检索/二次生成）前暂停等待人工确认，为 J20 未来 WebSocket human-in-the-loop 场景零重构接入做准备 | `agent.hitl.enabled`（默认 false） |
| D11 | run 内工具记忆化 | 同一 run 内 `(tool, query)` 规范 hash 命中即返回缓存结果，防止 Reflector 补检触发无效重复检索；生命周期即 run 本身，不跨 run 持久化 | `agent.tool_memo.enabled`（默认 true） |

> 编号说明：A/B 为本节新设的效率优化系列；D9'/D10' 加撇号以区分第九章已有的部署拓扑 (D9) 与安全设计 (D10)。
> 落地顺序建议：A2/D9'/D11（低成本高收益）→ B3/B4（资源防护）→ A1/B5（结构调整）→ D10'（远期预留）。全部项均不新增 LLM 调用，A2 与 D11 反而减少调用次数。

**项目结构**:
```
app/agent/
  graph.py              # LangGraph StateGraph 定义与编译（含 interrupt() HITL 预留挂点, D10'）
  routers.py            # 条件边路由函数集中定义（消费 3.4 带 ★ 字段; v2.2 新增）
  state.py              # AgentState TypedDict 定义
  research_subgraph.py  # 检索->融合->精排子图封装 (B5, ToolRouter 可调用单元; v2.2 新增)
  evidence_pruner.py    # 轮间证据修剪 (B3, checkpoint 写入前执行; v2.2 新增)
  nodes/
    planner.py          # 规划节点（输出 list[PlanStep]; 回环时增量补计划）
    tool_router.py      # 工具路由（A1 并行扇出 / D11 记忆化 / fan-in 合并）
    reflector.py        # 反思节点（C8 结构化输出 + A2 短路判定入口）
    generator.py        # 生成节点（D9' 上下文排序）
...
**核心裁决（v2 · M1，v2.2 按 J19/J20 修订落地方式）-- 缓冲式流（Buffered Streaming）**:

v1 同时声称"逐 Token 推送"与"忠实度不通过则回退重生成"——已推送的 Token 无法撤回，二者不可兼得。定案为缓冲式：
...
- v2.2 变化：后端"SSE 回放器"取消——langgraph-server 直接流出节点事件（thought 步骤经 stream_mode=updates 实时可看）与终态，模拟流式的打字机渲染职责移交前端
- 体验代价不变：用户看到首字的时刻 = 校验完成时刻（约 +1-3s），已计入 D4 延迟预算表
...
| Prompt 模板 | 系统 Prompt 强调"仅基于提供的证据回答，不确定时说不知道"；Web 检索内容以 XML 围栏隔离注入（防 Prompt 注入，见第九章 9.5） |
...
### 第 8 层：后处理层（Post-Processing Layer） -- **新增**
...
| 回退机制 | 忠实度低于阈值 → 带失败原因约束重新生成（受 self_correction_max_retries=1 限制，见第 6 层 M3）；重试耗尽则降级返回并标注 degraded=true。**回退发生在缓冲期内，客户端尚未收到任何 Token**（M1 裁决的前提） |
| 分级启用 | **v2 · M1 裁决：仅 deep 档位执行完整校验**；standard 档跳过（延迟考量）；fast 档完全跳过 |
...
**记忆架构（v2.1 · J17 定案）：Working Memory + Episodic Memory 混合架构**
...
| 语义缓存 L1 | Qdrant cache collection（v2 · H2 修正） | **向量 ANN 相似度匹配：查询向量与缓存问题向量检索 top-1，score ≥0.95 视为命中**。v1 的"Key=查询向量 hash"是精确匹配而非语义匹配（"今天天气"与"今日天气"的向量 hash 永不相等），已废弃。Redis 无原生向量检索能力，故迁移至 Qdrant 独立 collection（payload 存完整回答） |
...
app/memory/
  ...
  episodic.py           # 情景记忆：向量化存储与相关性检索 (v2.1 新增)
  scheduler.py          # 记忆调度器：注入决策 + 双重去重 (v2.1 新增)
...
| 自托管备选 (v2.2) | **Langfuse v4** —— 本机 Docker 环境已具备镜像（langfuse + langfuse-worker），作为 LangSmith SaaS 的开源自托管替代，二者二选一；数据不出内网场景优先选它 |
...
原始数据源
  ...
  -> [P7. 图谱构建层] 实体对齐 + 关系抽取 + 社区检测与摘要 (v2 新增)
```
...
| 策略 | 说明 |
|------|------|
| 全量扫描 vs 增量扫描 | 维护「已处理清单」（文件路径 + 内容哈希），只处理变更部分 |
...
**关键决策**: 大规模数据应支持增量加载。通过维护处理清单，只处理新增/变更文件。
...
**关键决策**: 至少保留标题层级和列表结构，以便后续分块时利用语义边界。

---

### P3. 清洗层（Cleaning） -- 重点细化
...
**关键参数（v2 · H3 修正：计量单位统一为字符）**:

> LangChain 的 `RecursiveCharacterTextSplitter` 按**字符数**计数，v1 中"chunk_size=500 (tokens)"的表述与实际实现不符。中文场景统一以字符为单位：1 汉字 ≈ 0.7-1.0 token（BGE-M3 tokenizer），500 字符 ≈ 350-500 token，仍在 BGE-M3 的 8192 窗口内。
...
**关键决策**: 块太小 -> 语义不完整，检索噪音大；太大 -> 超出 embedding 窗口，精度低。建议「结构优先 + 字符兜底」的多级策略。
...
**关键决策（v2.1 · J15 定案：类别白名单打底 + 访问计数叠加）**: LLM 语义增强效果好但成本高，判定哪些文档值得增强采用两套机制并集生效：
...
| 全文索引 | **v2.1 · J5 定案：移交外置 Elasticsearch 承担**（IK 中文分词），Neo4j 不再自建全文索引。v2 曾建议的 smartcn analyzer 方案随之废弃；ES 文档携带 entity_id/doc_id 映射字段支撑 J6 协同回查 |
| 属性索引 | 对频繁查询的属性（如 type, category）建 RANGE 索引 |

**全文索引（Elasticsearch，v2.1 新增）**:
| 配置项 | 说明 |
|--------|------|
...
**输出**: `VectorStore + GraphStore + FullTextIndex(ES, v2.1)`

---

### P7. 图谱构建层（Graph Construction） -- **v2 新增 (D3)**

> v1 的图侧设计只有"实体匹配 + 关系扩展"两句话，缺失从 NER 到可用图谱的整个上半场。本章补齐 GraphRAG 中"Graph"的完整构建链路。
...
**G1. 图 Schema 定义（v2.1 · J12 定案：混合式 = 白名单 + 开放区）**:
...
**G3. 关系抽取（v2.1 · J13 定案：本地轻量模型执行）**：

- 执行模型：**本地轻量模型**（如 Qwen2.5-7B 角色条目 extractor）——离线管道无实时压力，免费且数据不出境；质量不足时可通过 models.yaml 将该角色切换为任意更强的注册模型，管道代码零改动（J1/J2 红利）
...
| 更新策略 | 增量更新时仅重算受影响社区；全量重建阈值：变更实体占比 > 20% |
...
## 六、检索与评估层（Retrieval + Evaluation） -- 核心细化
...
| 策略 | 适用场景 | 实现 |
|------|----------|------|
| 密集向量检索 | 语义相似匹配 | Qdrant + BGE-M3 dense, cosine |
| 稀疏向量检索 | 关键词精确匹配 | Qdrant + BGE-M3 sparse, dot product |
| 图遍历检索 | 多跳推理、结构化知识 | Neo4j Cypher: 实体匹配 + 关系扩展 |
| 全文检索 | 补充稀疏检索 | Neo4j Lucene TEXT INDEX |
| Web 搜索 | 兜底外部知识 | Tavily/DuckDuckGo API |
| 元数据过滤 | 先过滤再检索，缩小搜索空间 | Qdrant filter + Neo4j WHERE |

**检索后处理**:
| 步骤 | 说明 |
|------|------|
| 结果归一化 | 统一各路分数到 [0,1] 区间 |
| RRF 融合 | Reciprocal Rank Fusion，按排名融合多路结果 |
| 加权融合 | 可配置权重（dense:0.4, sparse:0.2, graph:0.3, web:0.1） |
| 去重合并 | 基于文档 ID / 内容 hash 去重，同文档多 chunk 合并 |
| 上下文扩展 | 命中子块后，取父文档或相邻块补充上下文 |
| Re-ranking | BGE-Reranker-v2-m3 精排，阈值过滤，Top-K 截断 |

**路由决策**:
- 根据 query 类型路由到不同索引（事实型 -> 向量优先；关系型 -> 图谱 Local Search 优先；总结全局型 -> 社区摘要 Global Search）
- 根据 query 意图选择检索策略组合与延迟档位
- 多轮对话中的上下文继承（将历史对话融入当前查询）
...
**Golden Set 工程化 -- v2 新增 (D8)**:

评估指标只是公式，golden set 才是地基。v1 未说明测试集从哪来：
...
**Judge 模型定案**: RAGAS 默认依赖 OpenAI，本地部署改用 Qwen2.5-72B 作为 LLM-as-Judge；Faithfulness 可选轻量 NLI 模型（如 bge-reranker 变体或 mDeBERTa-nli）降低成本。
...
v1 存在四套配置源并存（`config.py` 环境变量 + `pipeline_config.yaml` + `cleaning_rules.yaml` + `chunking_config.yaml`），优先级与校验关系未定义。v2 定案，v2.1 补充模型注册表：
...
## 八、统一 Embedding 服务（BGE-M3）

BGE-M3 同时产出密集向量和稀疏向量，应封装为统一服务：
...
**Ollama 部署注意**: BGE-M3 通过 Ollama 运行时，密集向量和稀疏向量需要分别调用（Ollama 目前主要支持密集向量）。稀疏向量可通过 BGE-M3 的 ONNX 推理或专用库（如 `FlagEmbedding`）独立生成。

**async 集成定案（v2）**: `FlagEmbedding` 是同步库，在 FastAPI async 上下文中直接调用会阻塞事件循环。两种方式二选一：
...
项目结构补充 `flag_client.py`（进程内 FlagEmbedding 封装），与 `ollama_client.py` 并列实现同一 Protocol。
...
## 九、可靠性、部署与安全 -- **v2 新增 (D5/D6/D9/D10)**

### 9.1 多模型接入架构 -- **v2.1 新增 (J1/J2)**
...
- Embedding 与 Reranker **不进注册表**——固定本地 BGE-M3 + bge-reranker-v2-m3 进程内 FlagEmbedding 加载（J3），因二者与索引强绑定
...
docker-compose 单机全栈编排（J7），服务清单与版本约束：
...
| postgres | 17 | LangGraph thread checkpoint 存储（J21 会话状态权威源）；数据卷持久化 |
| qdrant | latest（本机镜像 2026-06 构建） | named sparse vectors 需 ≥1.10，本镜像满足；启动后经 `GET :6333` 确认小版本；REST 6333 / gRPC 6334；另承载语义缓存与情景记忆 collection |
| neo4j | 2026.06.0 community | 图谱权威数据源（J6）；数据卷持久化。2026 新版本线，Cypher 与 5.x 兼容；APOC/GDS 插件按 2026 版配套安装 |
| elasticsearch | 8.12.0 | IK 分词器插件需匹配 8.12.0 release（J5）；单节点模式即可 |
| redis | 7.x | AOF 持久化开启（工作记忆不丢失） |
| ollama (可选) | 宿主机原生安装（Windows），非容器化 | 默认 localhost:11434；仅当注册本地模型条目（Profile B）时启用 |

> **版本基准说明（v2.2）**：以上版本以本机 Docker 环境实测为准（2026-08-22 镜像清点）。项目根目录 `docker-compose.yml` 当前的服务清单与 Neo4j 版本声明（5.27-community）滞后于本文档，留待实施阶段统一对齐，本轮不改。
...
project_root/
  app/
    api/                    # 第1层：业务面 (v2.2 · J19)
      ...
      middleware.py         # 认证、限流、CORS 中间件
      schemas_api/          # 3.6 REST API 规范对应的请求/响应模型 (v2.2 新增)
    query/                  # 第2层：查询理解层
      router.py             # 意图分类
    ...
    retrieval/              # 第3层+第4层：检索+融合
      base.py               # BaseRetriever 协议 (v2 新增)
      dense_retriever.py    # Qdrant 密集向量检索
      sparse_retriever.py   # Qdrant 稀疏向量检索
      graph_retriever.py    # Neo4j 图遍历检索 (Local)
      global_retriever.py   # 社区摘要检索 (Global, v2 新增)
      es_retriever.py       # Elasticsearch 全文检索+图谱协同 (v2.1 更名)
      web_retriever.py      # Web 搜索工具（Tavily 主 + DDG 兜底双轨）
      fusion.py             # RRF / 加权融合
      normalizer.py         # 分数归一化
      deduplicator.py       # 结果去重
    reranking/              # 第5层：Reranker精排层
      ...
      scoring.py            # 分数计算与排序
    agent/                  # 第6层：Agent编排层
      graph.py              # LangGraph StateGraph（含 HITL interrupt 预留挂点）
      routers.py            # 条件边路由函数集中定义 (v2.2 新增)
      state.py              # AgentState 定义
      research_subgraph.py  # 检索->融合->精排子图封装 (v2.2 新增)
      evidence_pruner.py    # 轮间证据修剪 (v2.2 新增)
      nodes/
        planner.py
        tool_router.py
        reflector.py
    ...
    postprocessing/         # 第8层：后处理层
      ...
      formatter.py
    memory/                 # 第9层：记忆层
      working_memory.py     # 工作记忆：会话滑动窗口 (v2.1 更名自 conversation.py)
      episodic.py           # 情景记忆：向量化存储与检索 (v2.1 新增)
      scheduler.py          # 记忆调度器：注入决策+双重去重 (v2.1 新增)
      user_profile.py
      semantic_cache.py
    llm/                    # 多模型接入层 (v2.1 新增, J1/J2)
    ...
    embedding/              # 统一 Embedding 服务
      service.py            # BGE-M3 统一接口
      ollama_client.py      # Ollama API 封装（密集向量）
      flag_client.py        # FlagEmbedding 进程内封装（稀疏向量/reranker, v2 新增）
    core/                   # 核心配置与契约
      config.py             # pydantic-settings 配置（仅基础设施连接项）
      models.py             # 核心数据契约模型族 (v2 新增, 见第三章)
    db/                     # 数据库客户端封装
      neo4j_client.py
      qdrant_client.py
      redis_client.py
      es_client.py          # Elasticsearch 客户端 (v2.1 新增)
    main.py                 # FastAPI 应用入口
  config/                   # 配置文件目录（YAML 为业务策略单一事实来源, v2·D7）
    pipeline_config.yaml    # 数据管道配置
    cleaning_rules.yaml     # 清洗规则配置
    chunking_config.yaml    # 分块策略配置
    models.yaml             # 模型注册表 + 角色路由 + fallback 链 (v2.1 新增, J1/J2)
    graph_schema.yaml       # 图谱 Schema 白名单 + 开放区规则 (v2 新增, J12 混合式)
    entity_aliases.yaml     # 实体别名表 (v2 新增)
    reliability.yaml        # 超时与降级参数 (v2 新增)
  tests/                    # 测试
    golden/                 # 评估 golden set (v2 新增, 见第六章 D8)
    test_pipeline/          # 数据管道单元测试
    test_retrieval/         # 检索层测试
    test_agent/             # Agent 测试
  docker-compose.yml
  langgraph.json            # langgraph-server 配置: 图入口/依赖/环境 (v2.2 新增, J19)
  requirements.txt
```
...
建议按以下顺序分阶段实施（从高到低）。**v2 变更：插入阶段 0「契约先行」；可观测层从末位提前至阶段 3**——没有追踪的管道调试全靠 print，LangSmith tracing 必须在首个可运行链路时就接入：

0. **核心契约先行（v2 新增）** -- `app/core/models.py` 数据模型族、Retriever/Reranker/EmbeddingService Protocol、AgentState 字段表；`app/llm/` 多模型接入层与 `config/models.yaml` 注册表（J1/J2）；同时修正既有代码的类型漂移。没有这一步，后续各阶段并行推进会在接口上反复返工
1. **数据管道基础：P1采集 + P2解析 + P3清洗** -- 数据质量是一切的基础，清洗规则链优先搭建
2. **P4分块 + P5增强 + 统一 Embedding 服务 + P7图谱构建** -- 多级分块策略 + BGE-M3 双通道向量化 + 本地模型抽取与社区摘要（J13）
3. **P6索引（含 ES 同步）+ 多路检索层 + 融合层 + 可观测接入** -- Qdrant/Neo4j/ES 写入与基础检索能力（J5/J6 协同）；LangSmith tracing 同步接入（v2 提前）
4. **BGE-Reranker 精排层 + 可插拔上下文压缩** -- 最立竿见影的质量提升手段（H1 定案：FlagEmbedding 进程内；J11 策略可配置）
5. **Agent 编排层 + 生成层** -- 核心交互逻辑；实现 M1 缓冲式流、M3 终止预算、J9 全查询 Planner
6. **查询理解层** -- 单次结构化调用 + 延迟三档路由，进一步提升复杂查询表现
7. **后处理层（幻觉检测）+ 评估层** -- 质量兜底 + golden set 工程化 + CI 回归
...
10. **前端接入配套（v2.2 新增，J19-J22）** -- 按序四个子阶段：
    ...
    - **F4 图内节点适配**：新增 `load_memory` 前置节点与写入侧尾节点（工作记忆/情景记忆/precheck 缓存条目），对齐第二章双服务时序
