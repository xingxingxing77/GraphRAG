...
前端技术栈定案：React 19 + TypeScript 5 + Vite 6 + Tailwind CSS 4 + shadcn/ui + Zustand 5 + axios（REST）+ pnpm；AI 通信层 @langchain/langgraph-sdk；图可视化 @neo4j-nvl/react。
...
| GET | /config/public | 可选模型条目清单 + 延迟档位枚举 + 压缩策略枚举 + 运行 Profile（cloud-primary \| local-full，对应 D6 两档位）（J2「请求参数指定」的前端前提） | JWT |
...
| ALL | /admin/* 与 /admin/debug/* | 管理与调试接口组：缓存清理、索引重建、热更新、审核队列、任务进度、管道预览（parsing/cleaning/chunking）、检索调试（retrieve/rerank/embed/analyze）、社区与 points 查看、golden 导出；完整契约以开发文档套件 02 §3.10-§3.11 为准 | JWT+admin |
