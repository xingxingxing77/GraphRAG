...
# --- 本地模块 ---
from app.memory.conversation import WM_TTL_SECONDS, ConversationMemory
...
class TestConversationMemory:
    ...
    def memory(self) -> ConversationMemory:
        return ConversationMemory(FakeRedis(), max_turns=3)

    async def test_add_exchange_sets_wm_key_ttl_and_entry(self, memory: ConversationMemory) -> None:
        redis = memory.redis
    ...
    async def test_sliding_window_trims_to_max_turns(self, memory: ConversationMemory) -> None:
        for i in range(5):
    ...
    async def test_get_history_last_n_subset(self, memory: ConversationMemory) -> None:
        for i in range(3):
            await memory.add_exchange("s1", f"q{i}", f"a{i}")
        history = await memory.get_history("s1", last_n=2)
        assert [h["q"] for h in history] == ["q1", "q2"]

    async def test_clear_removes_key(self, memory: ConversationMemory) -> None:
